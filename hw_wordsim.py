...
if 'IS_GRADESCOPE_ENV' not in os.environ:
    giga20 = pd.read_csv(
        os.path.join(VSM_HOME, "giga_window20-flat.csv.gz"), index_col=0)
    ttested = ttest(giga20)
    result = full_word_similarity_evaluation(ttested)
    print(result)
...
"""
    The system builds the pipeline as:
    count_matrix-> ppmi-> lsa->  subword_enrichment_on_lsa-> weighted_lsa_representation-> MMD-VAE-> Retrofitting
    
    Steps:
    My hypothesis was that incoporating morphology information might be help pevent information loss because of the compression.
    I tried using ppmi->lsa->subword enrichment, it gave
    poor neighbours e.g:
                    finance    0.000000
                    balance    0.004174
                    chance     0.006348
                    advance    0.006587
                    glance     0.006653
    However a linear combination of ppmi and enrichment allows us to better incorporate morphological
    information hence I used a weihting strategy:
    weighted_ppmi_lsa_k = (w1*dataset_ppmi_lsa_k + w2*dataset_ppmi_lsa_k_ngram_enrichmet)/(w1+w2)
    
    This gave better resuts in finding neighbours:
                            finance        0.000000
                            banking        0.081684
                            investment     0.087688
    ...
    MMD-VAE (defined in torch_autoencoder_mmd_info.py) which tries to constrain the
    latent representation to unit Gaussian.
    I believe the advantage is that it should be robust to 
    different distance measures while also making the latent space more continous and meaningful
    
    
    After lsa and autoencoder we loose information because of the lower dimensional representation.
    So retrofitting is added to account for information loss.
    Also it helps mitigating issues which could arise because of dataset not being sufficiently representative    
...
if 'IS_GRADESCOPE_ENV' not in os.environ:
    ...
    if use_subword_enrichment:
        ...
        print(neighbours_retro)
        #print(X_retro)
        evaluation = full_word_similarity_evaluation(X_retro)
        print(evaluation)
